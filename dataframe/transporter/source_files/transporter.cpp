...
Transporter::Transporter(){
	MPI_Init_thread(NULL,NULL,MPI_THREAD_SERIALIZED,&mpi_threads_provided_);
	...
	logger.SetProcessInfo(
			process_info_.world_size,
			process_info_.world_rank,
			name
			);
...
std::string Transporter::GetSummary(){
	...
	summary.append(std::to_string(mpi_threads_provided_));
